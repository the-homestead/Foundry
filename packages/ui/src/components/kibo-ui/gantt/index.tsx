...
import { ContextMenu, ContextMenuContent, ContextMenuItem, ContextMenuTrigger } from "@foundry/ui/primitives/context-menu";
...
const getDateByMousePosition = (context: GanttContextProps, mouseX: number) => {
    const timelineYear = context.timelineData.at(0)?.year;
...
export interface GanttContentHeaderProps {
    ...
    getHeaderKey: (index: number) => string;
    title: string;
    columns: number;
}

export const GanttContentHeader: FC<GanttContentHeaderProps> = ({ title, columns, renderHeaderItem, getHeaderKey }) => {
    const id = useId();
        ...
        <div className="sticky top-0 z-20 grid w-full shrink-0 bg-backdrop/90 backdrop-blur-sm" style={{ height: "var(--gantt-header-height)" }}>
            ...
            >
                {Array.from({ length: columns }).map((_, index) => (
                    <div className="shrink-0 border-border/50 border-b py-1 text-center text-xs" key={`${id}-${getHeaderKey(index)}`}>
        ...
        year.quarters
            ...
            .map((month, index) => (
                <div className="relative flex flex-col" key={`${year.year}-${index}`}>
                    <GanttContentHeader
                        columns={month.days}
                        getHeaderKey={(item: number) => format(addDays(new Date(year.year, index, 1), item), "yyyy-MM-dd")}
                    ...
                    />
                    <GanttColumns columns={month.days} isColumnSecondary={(item: number) => [0, 6].includes(addDays(new Date(year.year, index, 1), item).getDay())} />
                </div>
            ))
    );
};
    ...
    return gantt.timelineData.map((year) => (
        <div className="relative flex flex-col" key={year.year}>
            <GanttContentHeader
                ...
                getHeaderKey={(item: number) => format(new Date(year.year, item, 1), "yyyy-MM")}
        ...
        year.quarters.map((quarter, quarterIndex) => (
            <div className="relative flex flex-col" key={`${year.year}-${quarterIndex}`}>
                <GanttContentHeader
                    ...
                    getHeaderKey={(item: number) => format(new Date(year.year, quarterIndex * 3 + item, 1), "yyyy-MM")}
...
export const GanttHeader: FC<GanttHeaderProps> = ({ className }) => {
    ...
    return (
        <div className={cn("flex h-full w-max -space-x-px divide-x divide-border/50", className)}>
            <Header />
        </div>
    );
};
...
export const GanttSidebarItem: FC<GanttSidebarItemProps> = ({ feature, onSelectItem, className }) => {
    ...
    return (
        // biome-ignore lint/a11y/useSemanticElements: "This is a clickable column"
        <div
...
export const GanttSidebarGroup: FC<GanttSidebarGroupProps> = ({ children, name, className }) => (
    <div className={className}>
        <p className="w-full truncate p-2.5 text-left font-medium text-muted-foreground text-xs" style={{ height: "var(--gantt-row-height)" }}>
            {name}
        </p>
...
export const GanttSidebar: FC<GanttSidebarProps> = ({ children, className }) => (
    <div className={cn("sticky left-0 z-30 h-max min-h-full overflow-clip border-border/50 border-r bg-background/90 backdrop-blur-md", className)} data-roadmap-ui="gantt-sidebar">
        <GanttSidebarHeader />
...
export const GanttAddFeatureHelper: FC<GanttAddFeatureHelperProps> = ({ top, className }) => {
    ...
    return (
        ...
        >
            <button className="flex h-full w-full items-center justify-center rounded-md border border-dashed p-2" onClick={handleClick} type="button">
...
export const GanttColumn: FC<GanttColumnProps> = ({ index, isColumnSecondary }) => {
    ...
    return (
        ...
        // biome-ignore lint/a11y/noNoninteractiveElementInteractions: <"This is a clickable column">
        <div
...
export const GanttColumns: FC<GanttColumnsProps> = ({ columns, isColumnSecondary }) => {
    const columnKeys = useMemo(() => Array.from({ length: columns }, () => crypto.randomUUID()), [columns]);
    ...
    return (
        ...
        >
            {columnKeys.map((key, index) => (
                <GanttColumn index={index} isColumnSecondary={isColumnSecondary} key={key} />
            ))}
        </div>
    );
};
...
export const GanttCreateMarkerTrigger: FC<GanttCreateMarkerTriggerProps> = ({ onCreateMarker, className }) => {
    ...
    return (
        <div className={cn("group pointer-events-none absolute top-0 left-0 h-full w-full select-none overflow-visible", className)} ref={mouseRef}>
            ...
            >
                <button className="z-50 inline-flex h-4 w-4 items-center justify-center rounded-full bg-card" onClick={handleClick} type="button">
...
export const GanttFeatureDragHelper: FC<GanttFeatureDragHelperProps> = ({ direction, featureId, date }) => {
    ...
    return (
        <div
            className={cn("group absolute top-1/2 z-3 h-full w-6 -translate-y-1/2 cursor-col-resize! rounded-md outline-none", direction === "left" ? "-left-2.5" : "-right-2.5")}
            ref={setNodeRef}
            {...attributes}
            {...listeners}
        >
...
export const GanttFeatureItem: FC<GanttFeatureItemProps> = ({ onMove, children, className, ...feature }) => {
    ...
    return (
        <div className={cn("relative flex w-max min-w-full py-0.5", className)} style={{ height: "var(--gantt-row-height)" }}>
            ...
            >
                {onMove && (
                    <DndContext modifiers={[restrictToHorizontalAxis]} onDragEnd={onDragEnd} onDragMove={handleLeftDragMove} sensors={[mouseSensor]}>
                        <GanttFeatureDragHelper date={startAt} direction="left" featureId={feature.id} />
                    </DndContext>
                )}
                <DndContext modifiers={[restrictToHorizontalAxis]} onDragEnd={onDragEnd} onDragMove={handleItemDragMove} onDragStart={handleItemDragStart} sensors={[mouseSensor]}>
                    <GanttFeatureItemCard id={feature.id}>{children ?? <p className="flex-1 truncate text-xs">{feature.name}</p>}</GanttFeatureItemCard>
                </DndContext>
                {onMove && (
                    <DndContext modifiers={[restrictToHorizontalAxis]} onDragEnd={onDragEnd} onDragMove={handleRightDragMove} sensors={[mouseSensor]}>
...
export const GanttFeatureRow: FC<GanttFeatureRowProps> = ({ features, onMove, children, className }) => {
    ...
    for (const feature of sortedFeatures) {
        ...
        let currentSubRowEnd = subRowEndTimes[subRow];

        while (subRow < subRowEndTimes.length && currentSubRowEnd && currentSubRowEnd > feature.startAt) {
            subRow++;
            currentSubRowEnd = subRowEndTimes[subRow];
        }
...
> = memo(({ label, date, id, onRemove, className }) => {
    ...
    const innerOffset = useMemo(() => calculateInnerOffset(date, gantt.range, (gantt.columnWidth * gantt.zoom) / 100), [date, gantt.range, gantt.columnWidth, gantt.zoom]);
        ...
        >
            <ContextMenu>
                <ContextMenuTrigger asChild>
                    ...
                    >
                        {label}
                        <span className="max-h-0 overflow-hidden opacity-80 transition-all group-hover:max-h-8">{formatDate(date, "MMM dd, yyyy")}</span>
                    </div>
                </ContextMenuTrigger>
                <ContextMenuContent>
                    {onRemove ? (
...
export const GanttProvider: FC<GanttProviderProps> = ({ zoom = 100, range = "monthly", onAddItem, children, className }) => {
    ...
    return (
        ...
        >
            <div
                className={cn("gantt relative isolate grid h-full w-full flex-none select-none overflow-auto rounded-sm bg-secondary", range, className)}
                ref={scrollRef}
                style={{
                    ...cssVariables,
