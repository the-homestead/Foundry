...
export const ColorPicker = ({ value, defaultValue = "#000000", onChange, className, ...props }: ColorPickerProps) => {
    ...
    useEffect(() => {
        if (value) {
            ...
            const { h = 0, s = 0, l = 0 } = color.hsl().object();
...
export const ColorPickerAlpha = ({ className, ...props }: ColorPickerAlphaProps) => {
    ...
    return (
        <Slider.Root
            ...
            max={100}
            onValueChange={(values) => {
                const [nextAlpha] = values;

                if (typeof nextAlpha === "number") {
                    setAlpha(nextAlpha);
                }
            }}
            step={1}
...
export const ColorPickerEyeDropper = ({ className, ...props }: ColorPickerEyeDropperProps) => {
    ...
    return (
        <Button className={cn("shrink-0 text-muted-foreground", className)} onClick={handleEyeDropper} size="icon" type="button" variant="outline" {...props}>
            <PipetteIcon size={16} />
        </Button>
    );
};
...
const PercentageInput = ({ className, ...props }: PercentageInputProps) => {
    return (
        <div className="relative">
            ...
            <span className="absolute top-1/2 right-2 -translate-y-1/2 text-muted-foreground text-xs">%</span>
        </div>
    );
};
    ...
    if (mode === "css") {
        ...
        return (
            <div className={cn("w-full rounded-md shadow-sm", className)} {...props}>
                <Input className="h-8 w-full bg-secondary px-2 text-xs shadow-none" readOnly type="text" value={`rgba(${rgb.join(", ")}, ${alpha}%)`} {...props} />
            </div>
        );
    }
