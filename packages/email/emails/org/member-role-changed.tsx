...
export const MemberRoleChanged = async ({ name, locale, orgName, role }: MemberRoleChangedProps) => {
    ...
    const previewText = t("organization.notice.role_changed", { details: `${role} · ${orgName ?? ""}` });
                ...
                <Body className="mx-auto my-auto px-2 font-sans" style={{ backgroundColor: EMAIL_COLORS.background }}>
                    ...
                    >
                        ...
                        <Text className="mb-[12px] text-center text-[14px]" style={{ color: EMAIL_COLORS.foreground }}>
                            {t("organization.notice.role_changed", { details: `${role ?? ""} · ${orgName ?? ""}` })}
                        </Text>
...
MemberRoleChanged.PreviewProps = { name: "Jane", orgName: "Foundry Labs", role: "Admin", locale: "en" } as MemberRoleChangedProps;

export default MemberRoleChanged;
