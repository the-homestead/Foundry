...
interface InviteCancelledProps {
    name?: string;
    locale?: Locale;
    orgName?: string;
}
...
export const InviteCancelled = async ({ name, locale, orgName }: InviteCancelledProps) => {
    ...
    const previewText = t("organization.inviteCancelled.preview", { orgName: orgName ?? "" });
                ...
                <Body className="mx-auto my-auto px-2 font-sans" style={{ backgroundColor: EMAIL_COLORS.background }}>
                    ...
                    >
                        <Heading className="mx-0 mb-[8px] text-center font-semibold text-[22px]" style={{ color: EMAIL_COLORS.foreground }}>
                            {t("organization.inviteCancelled.subject", { orgName: orgName ?? "" })}
                        </Heading>
                        ...
                        <Text className="mb-[12px] text-center text-[14px]" style={{ color: EMAIL_COLORS.foreground }}>
                            {t("organization.inviteCancelled.body", { orgName: orgName ?? "" })}
                        </Text>
...
InviteCancelled.PreviewProps = { name: "Jane", orgName: "Foundry Labs", locale: "en" } as InviteCancelledProps;
