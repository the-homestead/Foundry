...
export const SessionEnded = async ({ name, locale, device, ip }: SessionEndedProps) => {
    ...
    const previewText = t("sessionEnded.preview");
                ...
                <Body className="mx-auto my-auto px-2 font-sans" style={{ backgroundColor: EMAIL_COLORS.background }}>
                    ...
                    >
                        ...
                        <Text className="mb-[12px] text-center text-[14px]" style={{ color: EMAIL_COLORS.foreground }}>
                            {t("sessionEnded.body", { device: device ?? "—", ip: ip ?? "—" })}
                        </Text>
...
SessionEnded.PreviewProps = { name: "Jane", device: "Chrome on macOS", ip: "203.0.113.5", locale: "en" } as SessionEndedProps;
