...
export const LoginNotice = async ({ name, locale, ip, device, time, manageUrl }: LoginNoticeProps) => {
    ...
    const previewText = t("loginNotice.preview");
                ...
                <Body className="mx-auto my-auto px-2 font-sans" style={{ backgroundColor: EMAIL_COLORS.background }}>
                    ...
                    >
                        ...
                        <Text className="mb-[12px] text-center text-[14px]" style={{ color: EMAIL_COLORS.foreground }}>
                            {t("loginNotice.body", { device: device ?? "Unknown", ip: ip ?? "—", time: time ?? "—" })}
                        </Text>
...
LoginNotice.PreviewProps = {
    name: "Jane",
    ip: "203.0.113.5",
    device: "Chrome on macOS",
    time: "Feb 14, 2026 14:00 UTC",
    manageUrl: "https://example.com/account?tab=security",
    locale: "en",
} as LoginNoticeProps;
