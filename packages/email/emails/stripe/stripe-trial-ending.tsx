...
interface StripeTrialEndingProps {
    name?: string;
    locale?: Locale;
    plan?: string;
    days?: number;
...
export const StripeTrialEnding = async ({ name, locale, plan, days, manageUrl }: StripeTrialEndingProps) => {
    ...
    const previewText = t("stripe.trialEnding.preview");
                ...
                <Body className="mx-auto my-auto px-2 font-sans" style={{ backgroundColor: EMAIL_COLORS.background }}>
                    ...
                    >
                        <Heading className="mx-0 mb-[8px] text-center font-semibold text-[22px]" style={{ color: EMAIL_COLORS.foreground }}>
                            {t("stripe.trialEnding.subject", { days: days ?? 3 })}
                        </Heading>
                        ...
                        <Text className="mb-[12px] text-center text-[14px]" style={{ color: EMAIL_COLORS.foreground }}>
                            {t("stripe.trialEnding.body", { plan: plan ?? "", days: days ?? 3 })}
                        </Text>
...
StripeTrialEnding.PreviewProps = { name: "Jane", plan: "Pro", days: 3, manageUrl: "https://example.com/billing", locale: "en" } as StripeTrialEndingProps;

export default StripeTrialEnding;
