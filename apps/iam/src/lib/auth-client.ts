...
const publicAppUrl = process.env.NEXT_PUBLIC_APP_URL ?? "https://auth.homestead.systems";
...
export const authClient = createAuthClient({
    baseURL: publicAppUrl || undefined,
    fetchOptions: {
        credentials: "include",
    },
    plugins: [
...
export function signInWithProvider(provider: string) {
    ...
    const target = `/api/auth/oauth/${provider}?callbackURL=${encodeURIComponent(defaultCallbackUrl)}`;
