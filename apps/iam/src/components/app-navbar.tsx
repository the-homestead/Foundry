"use client";
import { signOut } from "@foundry/iam/lib/auth-client";
...
export function AppNavbar({ children, changeLocaleAction, locale }: { children: React.ReactNode; changeLocaleAction: (locale: Locale) => Promise<void>; locale: Locale }) {
    ...
    const navItems = [
        ...
        {
            name: t("items.foundry"),
            link: "https://foundry.homestead.systems",
        },
    ];
    ...
    // Desktop user/menu is handled by the shared NavUser (render props used for theme/locale)
    ...
    return (
        <div className="relative w-full">
            <Navbar>
                {/* Desktop Navigation */}
                <NavBody>
                    <NavbarLogo size="lg" title={t("brand.title")} url="/logo.png" />
                    <NavItems items={navItems} />
                    <div className="ml-auto flex items-center gap-3 justify-self-end">
                        <NavUser
                            onSignOut={() => signOut()}
                            renderLocaleControl={<LocaleSwitcher changeLocaleAction={changeLocaleAction} locale={locale} triggerClassName="h-8 w-full min-w-0" variant="navbar" />}
                            renderThemeControl={<ThemeSheet triggerClassName="h-8 w-8" />}
