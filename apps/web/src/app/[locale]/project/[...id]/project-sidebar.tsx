...
import { ArrowDownOnSquareIcon, ArrowTopRightOnSquareIcon, BookmarkIcon, LinkIcon } from "@foundry/ui/icons";
            ...
            <Card className="z-10 bg-sidebar sm:sticky sm:top-4">
                ...
                <CardContent>
                    <div className="space-y-3">
                        ...
                        <Button aria-label="Share project" className="w-full" size="sm" title="Share" variant="ghost">
                            ...
                            <div className="flex-1">
                                <CollapsibleTrigger asChild>
                                    ...
                                    <button className="flex w-full items-start justify-between gap-4 text-left">
                                        ...
                                        {/* icon */}
                                        <ChevronDownIcon className={`h-4 w-4 text-muted-foreground transition-transform ${isCompatOpen ? "rotate-180" : ""}`} />
                                    </button>
                                </CollapsibleTrigger>
                            </div>
                            <div className="ml-3">
                                {loading ? (
                                    ...
                                    <Stat
                                        label="Score"
                                        value={`${Math.round((project.compatibility.matrix.reduce((s, it) => s + it.value, 0) / Math.max(1, project.compatibility.matrix.length)) * 100)}%`}
                                    />
                                )}
                            </div>
                        </div>
                            ...
                            <div className="flex flex-col gap-2">
                                {links.map((link) => (
                                    <a
                                        ...
                                        className="flex items-center justify-between gap-3 rounded-md px-3 py-2 text-sm hover:bg-accent/30 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
                                        href={link.href}
                                        key={link.label}
                                    ...
                                    >
                                        ...
                                        <span className="flex h-6 w-6 items-center justify-center overflow-hidden rounded">
                                            <ArrowTopRightOnSquareIcon className="h-4 w-4 text-muted-foreground opacity-80" />
                                        </span>
                                    </a>
                                ))}
                            </div>
