    ...
    return (
        <div className="space-y-6">
            <div className="flex items-center gap-4">
                ...
                <div>
                    <h1 className="font-bold text-2xl tracking-tight">Edit Category</h1>
                    <p className="text-muted-foreground">
                        Update {category.name} for {game.name}
                    </p>
                </div>
            </div>

            <CategoryForm allCategories={allCategories} category={category} gameId={id} gameName={game.name} />
        </div>
    );
}
