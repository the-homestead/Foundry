...
import { ArrowDownOnSquareIcon, BookmarkIcon, ChatBubbleLeftRightIcon, ExclamationTriangleIcon, LinkIcon } from "@foundry/ui/icons";
...
export function ProjectSidebar({ project, links, creators, onDownload, onAddToCollection, loading = false, status }: ProjectSidebarProps) {
    ...
    return (
        <aside className="w-full space-y-4 rounded-md border border-border bg-card p-4 shadow-md ring-1 ring-border/5 sm:sticky sm:top-20 sm:w-72">
            ...
            {isUnapproved && (
                <Card className="w-full rounded-lg border-2 border-yellow-500 bg-yellow-50 dark:bg-yellow-950/20">
                    <CardHeader>
                        <CardTitle className="text-yellow-900 dark:text-yellow-100">
                            {/* Keep icon/text vertically aligned regardless of whether icon is rendered */}
                            <div className="flex items-center gap-2">
                                <ExclamationTriangleIcon className="h-5 w-5" />
                                <span className="leading-tight">UNAPPROVED PROJECT</span>
                            </div>
                        </CardTitle>
                    </CardHeader>
                    <CardContent>
                        <p className="text-sm text-yellow-800 dark:text-yellow-200">
                            This project is pending approval and is not publicly visible. Complete all required information and publish the project to make it visible to others. If
