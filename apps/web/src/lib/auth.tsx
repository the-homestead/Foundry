...
// import { userTable, eq } from "@foundry/database";
...
//         // sanitize incoming reset links (some callers accidentally include a
//         // leading `undefined` locale segment) and fall back to a safe URL.
...
//             const sanitizeUrl = (incoming?: string | null) => {
//                 try {
//                     if (!incoming) throw new Error("no url");
//                     const parsed = new URL(incoming, appUrl);
//                     const parts = parsed.pathname.split("/");
//                     if (parts[1] === "undefined") {
//                         parts.splice(1, 1);
//                         parsed.pathname = parts.join("/") || "/";
//                         console.warn("sanitized reset URL containing 'undefined' segment", { incoming, sanitized: parsed.toString() });
//                     }
...
//             const resetUrl = sanitizeUrl(url);
...
//         // plugin to attach passkey DB hooks (revalidation + passkey notice emails)
//         {
//             id: "foundry-passkey-hooks",
//             init(_ctx: any) {
//                 return ({
//                     options: {
//                         // Cast to any because `passkey` is not part of the public DB-hook types
//                         databaseHooks: {
//                             passkey: {
//                                 create: {
...
//                                                 from: fromEmail,
//                                                 to: toEmail,
//                                                 subject: (await createTranslator({ messages: await import(`../../messages/${locale}.json`), namespace: "emails", locale }))("passkey.subject", { action: "added" }),
//                                                 react: <PasskeyNotice locale={locale as any} action="added" device={device} manageUrl={`${appUrl}/account?tab=security`} name={toUser?.name ?? toEmail} />,
//                                             });
//                                         } catch (err) {
...
//                                                 from: fromEmail,
//                                                 to: toEmail,
//                                                 subject: (await createTranslator({ messages: await import(`../../messages/${locale}.json`), namespace: "emails", locale }))("passkey.subject", { action: "deleted" }),
//                                                 react: <PasskeyNotice locale={locale as any} action="deleted" device={device} manageUrl={`${appUrl}/account?tab=security`} name={toUser?.name ?? toEmail} />,
//                                             });
//                                         } catch (err) {
