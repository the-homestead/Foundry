...
//     try {
//         // Log callback URL & incoming state for debugging state_mismatch issues
//         const raw = c.req.raw as { url?: string } | undefined;
//         const rawUrl = raw?.url ?? c.req.url;
//         const url = rawUrl ? new URL(rawUrl, process.env.NEXT_SERVER_APP_URL ?? "http://localhost:3100") : null;
//         if (url?.pathname.includes("/callback")) {
//             const state = url.searchParams.get("state");
//             console.log("auth callback incoming:", { url: url.toString(), state });

//             if (state) {
//                 try {
//                     const verification = await db.query.verificationTable.findFirst({ where: eq(verificationTable.identifier, state) });
//                     console.log("auth callback: verification found:", verification ?? "none");
//                 } catch (err) {
//                     console.error("auth callback: verification lookup failed", err);
//                 }
//             }
//         }
//     } catch (_err) {
